// Code generated by mockery v2.16.0. DO NOT EDIT.

package mocks

import (
	echo "github.com/labstack/echo/v4"
	mock "github.com/stretchr/testify/mock"
)

// CompanyHandler is an autogenerated mock type for the CompanyHandler type
type CompanyHandler struct {
	mock.Mock
}

// EditProfile provides a mock function with given fields:
func (_m *CompanyHandler) EditProfile() echo.HandlerFunc {
	ret := _m.Called()

	var r0 echo.HandlerFunc
	if rf, ok := ret.Get(0).(func() echo.HandlerFunc); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(echo.HandlerFunc)
		}
	}

	return r0
}

// GetProfile provides a mock function with given fields:
func (_m *CompanyHandler) GetProfile() echo.HandlerFunc {
	ret := _m.Called()

	var r0 echo.HandlerFunc
	if rf, ok := ret.Get(0).(func() echo.HandlerFunc); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(echo.HandlerFunc)
		}
	}

	return r0
}

type mockConstructorTestingTNewCompanyHandler interface {
	mock.TestingT
	Cleanup(func())
}

// NewCompanyHandler creates a new instance of CompanyHandler. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewCompanyHandler(t mockConstructorTestingTNewCompanyHandler) *CompanyHandler {
	mock := &CompanyHandler{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
